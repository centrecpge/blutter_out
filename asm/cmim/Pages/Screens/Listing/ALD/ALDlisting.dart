...
class ALDlisting extends StatefulWidget {
  const ALDlisting({super.key});

  @override
  State<ALDlisting> createState() => _ALDlistingState();
}

class _ALDlistingState extends State<ALDlisting> {
  ...
  Future<void> getSts() async {
    // Similar to other modules, fetch ALD_status attributes
  }

  Future<void> getALD() async {
    try {
  ...
  Widget _buildBenTextField() {
    return TextFormField(
      ...
      onChanged: (value) {
        // Filter logic if client-side or re-fetch
      },
    );
  }
  ...
  Widget _buildList() {
    return ListView.builder(
      padding: const EdgeInsets.all(15),
      itemCount: aldList.length,
      itemBuilder: (context, index) {
        final item = aldList[index];
        return _buildAldItem(item);
      },
    );
  }

  Widget _buildAldItem(dynamic item) {
    return Card(
      ...
      child: Padding(
        ...
        child: Column(
          children: [
            ...
            _buildDetailRow("Pathologie", item['pathologie'] ?? ""),
            _buildDetailRow("Date début", item['date_debut'] ?? ""),
            _buildDetailRow("Date fin", item['date_fin'] ?? ""),
          ],
        ),
      ),
    );
  ...
  Widget _buildStatusChip(String status) {
    ...
    if (status.contains("Actif")) color = Colors.green;
    if (status.contains("Expiré")) color = Colors.red;
