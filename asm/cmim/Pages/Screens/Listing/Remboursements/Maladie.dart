...
class _MaladieState extends State<Maladie> {
  ...
  Future<void> getStorage() async {
    try {
      ...
      String? statusData = await storage.read(key: "stsListRmb");
      if (statusData != null) {
  ...
  Future<void> getRmb() async {
    try {
      ...
      final uri = Uri.https(
        "api.cmim.ma",
        "/api/Get_Reimbursements_Listing_mobile",
        queryParams,
      );
  ...
  Widget _buildList() {
    return ListView.builder(
      ...
      itemCount: rmbList.length,
      itemBuilder: (context, index) {
        final item = rmbList[index];
        return _buildRmbItem(item);
      },
    );
  }

  Widget _buildRmbItem(dynamic item) {
    return Card(
          ...
          children: [
            Row(
              ...
              children: [
                ...
                _buildStatusChip(item['statut_dossier'] ?? ""),
              ],
            ),
            const Divider(height: 20),
            _buildDetailRow("Bénéficiaire", item['beneficiaire'] ?? ""),
            _buildDetailRow("Acte", item['acte_principal'] ?? ""),
            _buildDetailRow("Montant engagé", "${item['montant_engage'] ?? "0"} DH"),
            _buildDetailRow("Montant remboursé", "${item['montant_rembourse'] ?? "0"} DH", isBold: true),
          ],
        ),
      ),
    );
  ...
  Widget _buildStatusChip(String status) {
    ...
    if (status.contains("Remboursé")) color = Colors.green;
    if (status.contains("En cours")) color = Colors.orange;
    if (status.contains("Rejeté")) color = Colors.red;
