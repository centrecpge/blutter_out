...
class _AccordsState extends State<Accords> {
  ...
  Future<void> getAcc() async {
    try {
      ...
      final uri = Uri.https(
        "api.cmim.ma",
        "/api/Get_Reimbursements_Accords_Listing_mobile",
        queryParams,
      );
  ...
  Widget _buildEmptyState() {
    return Center(
      child: Text("Aucun accord de remboursement trouvé", style: GoogleFonts.montserrat(color: Colors.grey)),
    );
  }
  ...
  Widget _buildList() {
    return ListView.builder(
      padding: const EdgeInsets.all(15),
      itemCount: accordList.length,
      itemBuilder: (context, index) {
        final item = accordList[index];
        return _buildAccordItem(item);
      },
    );
  }

  Widget _buildAccordItem(dynamic item) {
    return Card(
      ...
      child: Padding(
        ...
        child: Column(
          children: [
            ...
            _buildDetailRow("Montant", "${item['montant'] ?? "0"} DH"),
          ],
        ),
      ),
    );
